...
export default function DashboardPage() {
  ...
  const installedMatches = matches.filter((m) => m.status === "installed");
    ...
    <div className="min-h-[calc(100vh-160px)] bg-light">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-2 text-lg text-black-primary/50">
                {isOperator
                  ? "Here\u2019s an overview of your vending operations and opportunities."
                  : "Here\u2019s an overview of your location requests and operator connections."}
              </p>
            </div>
            <div className="flex items-center gap-2">
              <span className="inline-flex items-center gap-1.5 rounded-full bg-light-warm px-3 py-1.5 text-xs font-medium text-black-primary">
                {isOperator ? (
                  <><Building2 className="h-3.5 w-3.5 text-green-primary" /> Operator</>
                ) : profile.role === "location_manager" ? (
                  <><MapPin className="h-3.5 w-3.5 text-green-primary" /> Location Manager</>
                ) : (
                  <><User className="h-3.5 w-3.5 text-green-primary" /> Requestor</>
                )}
              </span>
              <button
                type="button"
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 space-y-8">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {isOperator ? (
            <>
              <Link href="/browse-requests" className="group flex items-center gap-4 rounded-2xl border border-gray-100 bg-white p-5 shadow-sm transition-all hover:-translate-y-0.5 hover:border-green-100 hover:shadow-md">
                ...
                <div>
                  <p className="font-semibold text-black-primary">Browse Requests</p>
                  <p className="text-sm text-black-primary/50">Find locations looking for vending machines</p>
                </div>
                <ChevronRight className="ml-auto h-5 w-5 text-black-primary/20 transition-colors group-hover:text-green-primary" />
              </Link>
              <Link href="/listings/new" className="group flex items-center gap-4 rounded-2xl border border-gray-100 bg-white p-5 shadow-sm transition-all hover:-translate-y-0.5 hover:border-green-100 hover:shadow-md">
                ...
                <div>
                  <p className="font-semibold text-black-primary">Create Listing</p>
                  <p className="text-sm text-black-primary/50">Advertise your vending services</p>
                </div>
                <ChevronRight className="ml-auto h-5 w-5 text-black-primary/20 transition-colors group-hover:text-blue-600" />
              </Link>
              <Link href="/messages" className="group flex items-center gap-4 rounded-2xl border border-gray-100 bg-white p-5 shadow-sm transition-all hover:-translate-y-0.5 hover:border-green-100 hover:shadow-md">
                <div className="relative flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-purple-50 text-purple-600 transition-colors group-hover:bg-purple-600 group-hover:text-white">
                  <MessageSquare className="h-6 w-6" />
                  {totalUnread > 0 && (
                    <span className="absolute -right-1 -top-1 flex h-5 min-w-[20px] items-center justify-center rounded-full bg-red-500 px-1 text-xs font-bold text-white">{totalUnread}</span>
                  )}
                </div>
                <div>
                  <p className="font-semibold text-black-primary">Messages</p>
                  <p className="text-sm text-black-primary/50">{totalUnread > 0 ? `${totalUnread} unread conversation${totalUnread > 1 ? "s" : ""}` : "View your conversations"}</p>
                </div>
            ...
            <>
              <Link href="/post-request" className="group flex items-center gap-4 rounded-2xl border border-gray-100 bg-white p-5 shadow-sm transition-all hover:-translate-y-0.5 hover:border-green-100 hover:shadow-md">
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-green-50 text-green-primary transition-colors group-hover:bg-green-primary group-hover:text-white">
                  <Plus className="h-6 w-6" />
                </div>
                <div>
                  <p className="font-semibold text-black-primary">Post a Request</p>
                  <p className="text-sm text-black-primary/50">Let operators know you need a vending machine</p>
                </div>
                <ChevronRight className="ml-auto h-5 w-5 text-black-primary/20 transition-colors group-hover:text-green-primary" />
              </Link>
              <Link href="/browse-operators" className="group flex items-center gap-4 rounded-2xl border border-gray-100 bg-white p-5 shadow-sm transition-all hover:-translate-y-0.5 hover:border-green-100 hover:shadow-md">
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-blue-50 text-blue-600 transition-colors group-hover:bg-blue-600 group-hover:text-white">
                  <Search className="h-6 w-6" />
                </div>
                <div>
                  <p className="font-semibold text-black-primary">Browse Operators</p>
                  <p className="text-sm text-black-primary/50">Find vending operators in your area</p>
                </div>
                <ChevronRight className="ml-auto h-5 w-5 text-black-primary/20 transition-colors group-hover:text-blue-600" />
              </Link>
              <Link href="/messages" className="group flex items-center gap-4 rounded-2xl border border-gray-100 bg-white p-5 shadow-sm transition-all hover:-translate-y-0.5 hover:border-green-100 hover:shadow-md">
        ...
        {isOperator ? (
          <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
            <StatCard icon={FileText} label="Applications" value={matches.length} color="orange" />
            <StatCard icon={CheckCircle2} label="Accepted" value={acceptedMatches.length} color="green" />
            <StatCard icon={Heart} label="Saved Requests" value={savedRequests.length} color="purple" />
            <StatCard icon={List} label="My Listings" value={listings.length} color="blue" />
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
            <StatCard icon={FileText} label="My Requests" value={requests.length} color="orange" />
            <StatCard icon={Handshake} label="Active Matches" value={pendingMatches.length + acceptedMatches.length} color="green" />
            <StatCard icon={Wrench} label="Installed" value={installedMatches.length} color="blue" />
            <StatCard icon={Eye} label="Total Views" value={requests.reduce((sum, r) => sum + (r.views || 0), 0)} color="purple" />
          </div>
        )}
          ...
          <>
            ...
            <section>
              <div className="mb-4 flex items-center justify-between">
                <h2 className="text-xl font-bold text-black-primary">My Applications</h2>
                <Link href="/browse-requests" className="inline-flex items-center gap-1.5 text-sm font-medium text-green-primary hover:underline">
              ...
              {loadingMatches ? (
                ...
                <EmptyState icon={FileText} title="No applications yet" description="Browse open requests and apply to locations that match your service area." cta="Browse Requests" href="/browse-requests" />
              ) : (
                <div className="space-y-3">
                  {matches.slice(0, 5).map((match) => (
                    <div key={match.id} className="group rounded-xl border border-gray-100 bg-white p-4 shadow-sm transition-all hover:border-green-100 hover:shadow-md sm:p-5">
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        <div className="flex items-start gap-3 min-w-0">
                          {match.vending_requests?.location_type && <LocationTypeIcon type={match.vending_requests.location_type} size="sm" />}
                          <div className="min-w-0">
                            <p className="truncate font-semibold text-black-primary">{match.vending_requests?.title || "Untitled Request"}</p>
                            <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-black-primary/50">
                              {match.vending_requests?.city && match.vending_requests?.state && (
                                <span className="flex items-center gap-1"><MapPin className="h-3 w-3" />{match.vending_requests.city}, {match.vending_requests.state}</span>
                              )}
                              <span className="flex items-center gap-1"><Calendar className="h-3 w-3" />{relativeDate(match.created_at)}</span>
                            </div>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                  ...
                  {matches.length > 5 && (
                    <p className="text-center text-sm text-black-primary/40">and {matches.length - 5} more application{matches.length - 5 > 1 ? "s" : ""}...</p>
                  )}
                </div>
              )}
            </section>
            ...
            <section>
              <div className="mb-4 flex items-center justify-between">
                <h2 className="text-xl font-bold text-black-primary">My Listings</h2>
                <Link href="/listings/new" className="inline-flex items-center gap-2 rounded-xl bg-green-primary px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-green-hover">
              ...
              {loadingListings ? (
                ...
                <EmptyState icon={List} title="No listings yet" description="Create a listing to let location managers know about your vending services." cta="Create a Listing" href="/listings/new" />
              ) : (
                <div className="grid gap-4 sm:grid-cols-2">
                  {listings.map((listing) => (
                    <div key={listing.id} className="group rounded-xl border border-gray-100 bg-white p-5 shadow-sm transition-all hover:border-green-100 hover:shadow-md">
                      <div className="mb-3 flex items-start justify-between">
                        <h3 className="font-semibold text-black-primary">{listing.title}</h3>
                        <div className="flex items-center gap-1">
                          <button type="button" onClick={() => setEditListing(listing)} className="rounded-lg p-1.5 text-black-primary/40 hover:bg-green-50 hover:text-green-primary cursor-pointer" aria-label="Edit listing"><Edit3 className="h-4 w-4" /></button>
                          <button type="button" onClick={() => setConfirmDelete({ type: "listing", id: listing.id, title: listing.title })} className="rounded-lg p-1.5 text-black-primary/40 hover:bg-red-50 hover:text-red-500 cursor-pointer" aria-label="Delete listing"><Trash2 className="h-4 w-4" /></button>
                        </div>
                      </div>
                      <div className="flex flex-wrap items-center gap-2 text-xs text-black-primary/50">
                        <span className="inline-flex items-center gap-1 rounded-full bg-gray-50 px-2 py-0.5"><Building2 className="h-3 w-3" />{listing.machine_count_available} machines</span>
                        <span className="inline-flex items-center gap-1 rounded-full bg-gray-50 px-2 py-0.5"><MapPin className="h-3 w-3" />{listing.states_served.join(", ")}</span>
                        <span className="inline-flex items-center gap-1 rounded-full bg-gray-50 px-2 py-0.5"><Eye className="h-3 w-3" />{listing.views} views</span>
                      </div>
                      {listing.machine_types.length > 0 && (
                        <div className="mt-3 flex flex-wrap gap-1.5">
                          {listing.machine_types.slice(0, 3).map((type) => (
                            <span key={type} className="rounded-full bg-green-50 px-2 py-0.5 text-xs font-medium text-green-primary">{type.replace(/_/g, " ")}</span>
                          ))}
                          {listing.machine_types.length > 3 && (
                            <span className="rounded-full bg-gray-50 px-2 py-0.5 text-xs text-black-primary/40">+{listing.machine_types.length - 3} more</span>
                          )}
                        </div>
                      )}
                    </div>
            ...
            {savedRequests.length > 0 && (
              <section>
                <h2 className="mb-4 text-xl font-bold text-black-primary">Saved Requests</h2>
                <div className="space-y-2">
                  {savedRequests.slice(0, 5).map((req) => (
                    <Link key={req.id} href={`/requests/${req.id}`} className="group flex items-center gap-4 rounded-xl border border-gray-100 bg-white p-4 shadow-sm transition-all hover:border-green-100 hover:shadow-md">
                      <LocationTypeIcon type={req.location_type} size="sm" />
                      <div className="min-w-0 flex-1">
                        <p className="truncate font-semibold text-black-primary group-hover:text-green-primary">{req.title}</p>
                        <p className="mt-0.5 text-xs text-black-primary/50">{req.city}, {req.state}</p>
                      </div>
        ...
        {/* ===================================================================
            LOCATION MANAGER / REQUESTOR SECTIONS
            =================================================================== */}
        {!isOperator && (
          <>
            {/* --- My Requests --- */}
            <section>
              <div className="mb-4 flex items-center justify-between">
                <h2 className="text-xl font-bold text-black-primary">My Requests</h2>
                <Link href="/post-request" className="inline-flex items-center gap-2 rounded-xl bg-green-primary px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-green-hover">
                  <Plus className="h-4 w-4" /> Post New Request
                </Link>
              </div>
              {loadingRequests ? (
                <div className="space-y-3"><SkeletonRow /><SkeletonRow /><SkeletonRow /></div>
              ) : requests.length === 0 ? (
                <EmptyState icon={FileText} title="No requests posted yet" description="Post your first request to let vending operators know about your location." cta="Post a Request" href="/post-request" />
              ) : (
                <div className="space-y-3">
                  {requests.map((req) => (
                    <div key={req.id} className="group rounded-xl border border-gray-100 bg-white p-4 shadow-sm transition-all hover:border-green-100 hover:shadow-md sm:p-5">
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        <div className="flex items-start gap-3 min-w-0">
                          <LocationTypeIcon type={req.location_type} size="sm" />
                          <div className="min-w-0">
                            <p className="truncate font-semibold text-black-primary">{req.title}</p>
                            <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-black-primary/50">
                              <span className="flex items-center gap-1"><MapPin className="h-3 w-3" />{req.city}, {req.state}</span>
                              <span className="flex items-center gap-1"><Eye className="h-3 w-3" />{req.views} views</span>
                              <span className="flex items-center gap-1"><Calendar className="h-3 w-3" />{relativeDate(req.created_at)}</span>
                            </div>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            className={`inline-flex items-center gap-1 rounded-lg px-2.5 py-1.5 text-xs font-medium transition-colors cursor-pointer ${req.status === "open" ? "bg-slate-50 text-slate-600 hover:bg-slate-100" : "bg-emerald-50 text-emerald-600 hover:bg-emerald-100"}`}
                          >
                            {req.status === "open" ? <><XCircle className="h-3.5 w-3.5" /> Close</> : <><CheckCircle2 className="h-3.5 w-3.5" /> Reopen</>}
                          </button>
                          <button type="button" onClick={() => setConfirmDelete({ type: "request", id: req.id, title: req.title })} className="rounded-lg p-1.5 text-black-primary/30 hover:bg-red-50 hover:text-red-500 cursor-pointer" aria-label="Delete request"><Trash2 className="h-4 w-4" /></button>
                          <Link href={`/requests/${req.id}`} className="inline-flex items-center gap-1.5 rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-xs font-medium text-black-primary hover:border-green-200 hover:bg-green-50 hover:text-green-primary">View <ChevronRight className="h-3.5 w-3.5" /></Link>
                        </div>
                      </div>
                    </div>
                  ))}
            ...
            <section>
              <div className="mb-4">
                <h2 className="text-xl font-bold text-black-primary">Operator Matches</h2>
                <p className="mt-1 text-sm text-black-primary/50">Operators who have applied to your requests</p>
              </div>
              {loadingMatches ? (
                <div className="space-y-3"><SkeletonRow /><SkeletonRow /></div>
              ) : matches.length === 0 ? (
                <EmptyState icon={Users} title="No matches yet" description="Once operators apply to your requests, they will appear here." />
              ) : (
                <div className="space-y-3">
                  {matches.map((match) => {
                        ...
                        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                          <div className="flex items-center gap-3 min-w-0">
                            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-green-primary text-sm font-bold text-white">
                              {operator?.full_name?.split(" ").map((w: string) => w[0]).join("").slice(0, 2).toUpperCase() || "??"}
                            </div>
                            <div className="min-w-0">
                              <div className="flex items-center gap-2">
                                <p className="truncate font-semibold text-black-primary">{operator?.full_name || "Unknown Operator"}</p>
                                {operator?.verified && <CheckCircle2 className="h-4 w-4 shrink-0 text-blue-500" />}
                              </div>
                              <div className="mt-0.5 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-black-primary/50">
                                {operator?.company_name && <span>{operator.company_name}</span>}
                                {match.vending_requests?.title && <span className="flex items-center gap-1"><FileText className="h-3 w-3" />{match.vending_requests.title}</span>}
                              </div>
                              {operator?.rating !== undefined && operator.rating > 0 && (
                                <div className="mt-1"><StarRating value={operator.rating} size="sm" /></div>
                              )}
                            </div>
                          </div>
                          <div className="flex items-center gap-2">
                            ...
                            {match.status === "pending" && (
                              <>
                                <button type="button" onClick={() => handleMatchAction(match.id, "accepted")} disabled={actionLoading === match.id} className="inline-flex items-center gap-1.5 rounded-lg bg-emerald-50 px-3 py-1.5 text-xs font-medium text-emerald-700 hover:bg-emerald-100 disabled:opacity-50 cursor-pointer">
                                  {actionLoading === match.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <CheckCircle2 className="h-3.5 w-3.5" />} Accept
                                </button>
                                <button type="button" onClick={() => handleMatchAction(match.id, "declined")} disabled={actionLoading === match.id} className="inline-flex items-center gap-1.5 rounded-lg bg-red-50 px-3 py-1.5 text-xs font-medium text-red-600 hover:bg-red-100 disabled:opacity-50 cursor-pointer">
                                  <XCircle className="h-3.5 w-3.5" /> Decline
                                </button>
                              </>
                            )}
                            {match.status === "accepted" && (
                              <button type="button" onClick={() => handleMatchAction(match.id, "installed")} disabled={actionLoading === match.id} className="inline-flex items-center gap-1.5 rounded-lg bg-blue-50 px-3 py-1.5 text-xs font-medium text-blue-700 hover:bg-blue-100 disabled:opacity-50 cursor-pointer">
                                {actionLoading === match.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Wrench className="h-3.5 w-3.5" />} Mark Installed
                              </button>
                            )}
                            {match.status === "installed" && (
                              <button type="button" onClick={() => setReviewTarget({ matchId: match.id, operatorId: match.operator_id, operatorName: operator?.full_name || "Operator" })} className="inline-flex items-center gap-1.5 rounded-lg bg-amber-50 px-3 py-1.5 text-xs font-medium text-amber-700 hover:bg-amber-100 cursor-pointer">
          ...
          <section>
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-xl font-bold text-black-primary">
                Recent Messages
                {totalUnread > 0 && (
                  <span className="ml-2 inline-flex h-5 min-w-[20px] items-center justify-center rounded-full bg-green-primary px-1.5 text-xs font-bold text-white">{totalUnread}</span>
                )}
              </h2>
              <Link href="/messages" className="inline-flex items-center gap-1.5 text-sm font-medium text-green-primary hover:underline">
                Open full inbox <ExternalLink className="h-3.5 w-3.5" />
              </Link>
            </div>
            <div className="space-y-2">
              {conversations.slice(0, 4).map((convo) => (
                <Link key={convo.partner.id} href={`/messages?with=${convo.partner.id}`} className="group flex items-center gap-3 rounded-xl border border-gray-100 bg-white p-4 shadow-sm transition-all hover:border-green-100 hover:shadow-md">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-green-primary text-sm font-bold text-white">
                    {convo.partner.full_name?.split(" ").map((w) => w[0]).join("").slice(0, 2).toUpperCase() || "??"}
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between">
                      <p className={`truncate text-sm font-semibold ${convo.unread > 0 ? "text-black-primary" : "text-black-primary/70"}`}>{convo.partner.full_name}</p>
                      <span className="ml-2 shrink-0 text-xs text-black-primary/40">{relativeDate(convo.lastDate)}</span>
                    </div>
                    <p className={`mt-0.5 truncate text-xs ${convo.unread > 0 ? "font-medium text-black-primary/70" : "text-black-primary/40"}`}>{convo.lastMessage}</p>
                  </div>
                  {convo.unread > 0 && (
                    <span className="flex h-5 min-w-[20px] items-center justify-center rounded-full bg-green-primary px-1.5 text-xs font-bold text-white">{convo.unread}</span>
                  )}
                </Link>
              ))}
            </div>
        ...
        {/* ------- PROFILE ------- */}
        <section>
          <h2 className="mb-4 text-xl font-bold text-black-primary">Profile Settings</h2>
          <ProfileForm profile={profile} onSave={handleProfileSave} saving={savingProfile} />
        </section>
      </div>

      {/* ------- TOASTS ------- */}
      {profileToast && <div className="toast toast-success">{profileToast}</div>}
      {reviewToast && <div className="toast toast-success">{reviewToast}</div>}
      ...
      />
      {editListing && (
        <EditListingModal listing={editListing} open={!!editListing} saving={savingListing} onSave={handleSaveListing} onCancel={() => setEditListing(null)} />
      )}
      {reviewTarget && (
        <ReviewModal open={!!reviewTarget} operatorName={reviewTarget.operatorName} saving={savingReview} onSubmit={handleSubmitReview} onCancel={() => setReviewTarget(null)} />
      )}
    </div>
  );
}
