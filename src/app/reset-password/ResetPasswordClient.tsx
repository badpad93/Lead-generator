...
export default function ResetPasswordClient({
  ...
  const brandSlug = initialBrand?.slug ?? null;
  // After a successful reset, land customers on their operator's
  // storefront; everyone else on the dashboard.
  const doneRedirect = brandSlug ? `/coffee/o/${brandSlug}` : "/dashboard";
      ...
      if (accessToken && refreshToken) {
        ...
        supabase.auth
          ...
          .then(() => {
            ...
            // Preserve the ?storefront= query while dropping the token hash.
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // The middleware customer-lock re-routes an enrolled customer to
      // their storefront regardless, so this is a fast-path only.
      setTimeout(() => {
  ...
  return (
    <div className="min-h-[calc(100vh-160px)] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        <AuthBrandHeader brand={initialBrand} />
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
          <div className="text-center mb-6">
            ...
            <p className="text-sm text-black-primary/60">
              {done
                ? "Your password has been changed. Taking you where you need to go..."
          ...
          {done ? (
            <div className="flex flex-col items-center gap-3">
              ...
              <Link href={doneRedirect} className="text-sm text-green-primary hover:underline">
                Continue
              </Link>
            </div>
          ) : !sessionReady ? (
