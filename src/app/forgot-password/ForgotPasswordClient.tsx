...
export default function ForgotPasswordClient({
  ...
  const loginHref = brandSlug
    ? `/login?storefront=${encodeURIComponent(brandSlug)}`
    : "/login";
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Carry the operator context into the reset link so the
      // /reset-password screen the email lands on wears the same brand.
      // Supabase appends its recovery token as a URL hash, which
      // coexists with this query string. Branding only — never auth.
      const resetPath = brandSlug
        ? `/reset-password?storefront=${encodeURIComponent(brandSlug)}`
        : "/reset-password";
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
          ...
          <div className="mt-6 text-center">
            <Link href={loginHref} className="text-sm text-black-primary/60 hover:text-black-primary">
              Back to login
            </Link>
          </div>
        </div>
