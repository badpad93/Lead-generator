-- Coffee Supply Agreement — version 3.
--
-- Bumps the Minimum Purchase Requirement from $200/machine/month
-- (v2, migration 159) to $1,000/machine/month per the customer's
-- updated commercial terms. Everything else on the template is
-- unchanged from v2.
--
-- Anyone who signed v1 or v2 keeps their historical user_agreements
-- row untouched — the versioning system is exactly here to preserve
-- what was signed at what time. New signers land on v3 via
-- getActiveTemplate('coffee_supply').

-- Deactivate v2 so getActiveTemplate('coffee_supply') resolves to v3.
...
INSERT INTO agreement_templates (
  ...
  <p>Customer agrees to purchase a minimum of One Thousand Dollars ($1,000.00) per machine per month in beverage supplies from Company. Failure to meet this requirement may result in termination of this Agreement and removal of Equipment.</p>
  ...
  <ul>
    ...
    <li>Customer acknowledges the minimum purchase requirement of $1,000 per machine per month.</li>
